 ...
 *
 * Argos SMD SPI DFU Test - Full DFU functionality test
 */
...
#else
/* Minimal test firmware - valid STM32 vector table stub */
static const uint8_t test_firmware[] = {
	0x00, 0x20, 0x00, 0x08,  /* Initial SP */
...
static int test_dfu_erase(void)
{
	LOG_INF("--- DFU ERASE ---");
...
static int test_dfu_write_chunk(void)
{
...
static int test_full_firmware_update(void)
{
...
bootloader_ready:
	/* ===== STEP 7: GET BOOTLOADER INFO ===== */
	{
		...
		if (ret == 0) {
			...
			LOG_INF("App start: 0x%08X, max size: %u, page size: %u",
	...
	/* ===== STEP 8: ERASE FLASH ===== */
	LOG_INF("");
	LOG_INF("=== STEP 8: ERASE FLASH ===");
	ret = test_dfu_erase();
	if (ret != 0) {
		LOG_ERR("Erase failed, aborting test");
		goto done;
	}

	/* ===== STEP 9: WRITE TEST CHUNK ===== */
	LOG_INF("");
	LOG_INF("=== STEP 9: WRITE TEST CHUNK ===");
	ret = test_dfu_write_chunk();
	if (ret != 0) {
		LOG_ERR("Write chunk failed: %d", ret);
	}

	/* ===== STEP 10: FULL FIRMWARE UPDATE ===== */
	LOG_INF("");
	LOG_INF("=== STEP 10: FULL FIRMWARE UPDATE ===");
	LOG_INF("Firmware size: %u bytes", TEST_FW_SIZE);
	ret = test_full_firmware_update();
	if (ret != 0) {
		LOG_ERR("Firmware update failed: %d", ret);
	} else {
		LOG_INF(">> FIRMWARE UPDATE SUCCESS!");
	}
